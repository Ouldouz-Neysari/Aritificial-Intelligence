...
class Sim:
    ...
    def _evaluate(self):
        eva=0
        gameover_check=self.gameover(self.red,self.blue)
        if (gameover_check=='red'):
            return -math.inf
        elif(gameover_check=='blue'):
            return math.inf
        else:
            for i in range(len(self.available_moves)):
                if(self.turn=='red'):
                    ...
                    go_var=self.gameover(self.red,self.blue)
                    if(go_var==0):
                        eva+=4
                    if(go_var=='blue'):
                        eva-=2
                    self.available_moves.insert(i,first_move)
                    self.red.remove(first_move)
                else:
                    first_move=self.available_moves.pop(i)
                    ...
                    if(go_var=='red'):
                        eva+=2
                    self.available_moves.insert(i,first_move)
                    lst_move=self.blue.remove(first_move)            
    ...
    def minimax(self,depth,player_turn):
        ...
        for i in range(len(self.available_moves)):
            ...
            if(player_turn=='red'):
                self.red.append(first_move)
                the_move, the_score=self.minimax(depth-1,'blue')
                self.available_moves.insert(i,first_move)
                self.red.remove(first_move)
                ...
                self.blue.append(first_move)
                the_move, the_score=self.minimax(depth-1,'red')
                self.available_moves.insert(i,first_move)
                self.blue.remove(first_move)
                if (the_score <= best_score):
                    best_score=the_score
                    selected_move=first_move
