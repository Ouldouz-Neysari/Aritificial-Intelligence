...
class State():
  def __init__(self,start_node,_path,node_time,rcp_dict,morid_dict,_cost,ps_num):
...
def add_children_l(cur_state,frontier,visited,limit):
  cur_node=cur_state.position

  for nd in cur_node.adjs :
    if(len(cur_state.path) >= limit):
      return False
    elif(cur_state.path==[] or cur_state.path==None):
      ns_path=[cur_state]
    ...
    if(nd.diff==True):
      ...
      _ns_time=0
      ns_pass=0
    ns=State(nd,ns_path,_ns_time,ns_vis_rcp,ns_vis_mrd,ns_cost,ns_pass)
    ...
    for st1 in frontier:
      if(st1.position.name ==ns.position.name and st1.visited_recp==ns.visited_recp and st1.visited_morid==ns.visited_morid ):
        current_visited=True
...
def limited_DFS(S,G,l):
    ...
    while True:
      ...
      if(current_state.needed_time!=0):
        ...
        suc=add_children_l(current_state,frontier,visited,l)
...
Init_state=State(start_node,passed_path,nd_tm, rcps_dict,morid_dict,0,0)


Ex,final_state,total_cost,total_visited_states=IDS(Init_state,G)
cur_time_=time.time()
