...
def Astar_add_children(cur_state,frontier,visited,G):
  cur_node=cur_state.position
...
def A_star(S,G):
  ...
  while True:
    ...
    if(current_state.needed_time!=0):
        ...
        heapq.heappush(frontier,(det_Function(current_state),current_state))
    else:
        Astar_add_children(current_state,frontier,visited,G)
...
Ex,final_state,total_cost,total_visited_states=A_star(Init_state,G)
cur_time_=time.time()
